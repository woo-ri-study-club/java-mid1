...
### 트랙잭션 롤백 처리에서의 체크 예외와 언체크 예외의 차이 
```
1. 언체크드 예외(RuntimeException과 그 하위)
- 트랙잭션이 롤백됩니다.
- 시스템 예외나 프로그래밍 오류와 같이 복구가 불가능한 예외로 간주되기 때문입니다.
- 예) NullPointerException, IllegalArgumentException

2. 체크드 예외(Exception과 그 하위, RuntimeException 제외)
- 트랜잭션이 롤백되지 않고 커밋됩니다.
- 예상되는 예외 상황으로 간주되기 때문입니다.
- 예) SQLException, IOException
 
실제 개발에서는 대부분의 예외는 언체크드 예외로 처리하는 것이 권장된다고 한다.
```
